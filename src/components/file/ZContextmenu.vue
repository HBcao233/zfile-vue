<template>
  <Contextmenu ref="contextmenu" auto-ajust-placement @show="addUnderlineToLastVisibleItem">
    <template v-if="contextMenuTargetFile">
      <div class="contextmenu-group-item">
        ...
        <ContextmenuItem
          v-show="
            storageConfigStore.globalConfig.kkFileViewUrl &&
            storageConfigStore.permission.preview &&
            selectStatistics.isSingleSelectFile
          "
        ...
        <ContextmenuItem
          v-show="selectStatistics.isAllFile && storageConfigStore.permission.copyDownloadLink"
      ...
      <div class="contextmenu-group-item">
        <ContextmenuItem
          v-show="selectStatistics.isAllFile && storageConfigStore.permission.pathLink"
        ...
        <ContextmenuItem
          v-show="selectStatistics.isAllFile && storageConfigStore.permission.shortLink"
        ...
        <ContextmenuItem
          v-show="storageConfigStore.permission.bothLink"
      ...
      <div class="contextmenu-group-item">
        ...
        <ContextmenuItem
          v-if="
            storageConfigStore.permission.move &&
            (selectStatistics.isAllFolder || selectStatistics.isAllFile)
          "
          @click="moveTo"
        >
        ...
        <ContextmenuItem
          v-if="
            storageConfigStore.permission.copy &&
            (selectStatistics.isAllFolder || selectStatistics.isAllFile)
          "
          @click="copyTo"
        >
      ...
      <div class="contextmenu-group-item">
        <ContextmenuItem v-show="storageConfigStore.permission.compress" @click="compress">
          <i-mdi-zip-box-outline class="contextmenu-icon" />
          <label>压缩...</label>
        </ContextmenuItem>
        <ContextmenuItem
          v-show="storageConfigStore.permission.decompress && isArchive"
          @click="decompress"
        >
    ...
    <div class="contextmenu-group-item">
      ...
      <ContextmenuItem
        v-show="storageConfigStore.permission.uploadFolder"
...
import { removeDuplicateSeparator, getFileSuffix } from '~/utils';
...
const { rename, batchDownloadFile, moveTo, copyTo, newFolder, batchDelete, compress, decompress } =
  useFileOperator();
...
const isArchive = computed(() => {
  ...
  if (!row || selectRows.value.length !== 1 || row.type !== 'FILE') {
    return false;
  }
  const suffix = getFileSuffix(row.name || '');
  return constant.fileTypeMap['archive'].includes(suffix);
});
...
.contextmenu-divider {
  @apply mb-2 border-b border-gray-300;
}
</style>
<style>
.compress-message-box .el-message-box__message {
  @apply w-full;
}
</style>
