  ...
  >
    <template #empty>
      <div v-show="!basicLoading">
        ...
        <div class="text-base font-bold">数据为空，请先上传或添加文件</div>
      </div>
    </template>
        ...
        <div v-show="skeletonLoading">
          <el-skeleton animated>
            <template #template>
              ...
              />
              <el-skeleton-item variant="text" style="vertical-align: middle; width: 30%" />
            </template>
          </el-skeleton>
        </div>
        <div v-show="!skeletonLoading">
...
.el-table {
  ...
  .el-table__header-wrapper svg {
    @apply -mt-0.5 mr-2 inline w-3;
  }

  /* 表身 -- 文件名列 icon 位置 */
  .el-table__body-wrapper .zfile-table-col-name svg {
    @apply relative mr-1.5 inline align-middle text-xl;
  }
}
