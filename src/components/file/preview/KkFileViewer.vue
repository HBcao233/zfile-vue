<template>
  <div class="zfile-kkfile-viewer">
    <div v-if="isLoading" class="viewer-status">正在加载预览, 请稍候...</div>

    <div v-else-if="errorMessage" class="viewer-status error">加载预览失败：{{ errorMessage }}</div>

    <iframe v-else-if="iframeSrc" :src="iframeSrc" width="100%" height="100%" frameborder="0">
    </iframe>
  </div>
</template>
