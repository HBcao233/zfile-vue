<template>
  <div>
    ...
    <div v-if="currentVideo && isMobile" class="zfile-video-switch-tools">
      <el-button
...
let nextVideoIcon =
  '<i class="art-icon art-icon-prev-video"><svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24"><path fill="currentColor" d="m6 18l8.5-6L6 6v12zM16 6v12h2V6h-2z"/></svg></i>';
let prevVideoIcon =
  '<i class="art-icon art-icon-next-video"><svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24"><path fill="currentColor" d="M6 6h2v12H6zm3.5 6l8.5 6V6z"/></svg></i>';
...
.artplayer-app {
  ...
  :deep(.art-controls .art-control) {
    @apply min-h-[14px] min-w-[14px] sm:min-h-[46px] sm:min-w-[46px];
    .art-icon {
      width: 28px;
    }
  }
...
.zfile-video-switch-tools {
  @apply flex justify-between bg-gray-50 px-5 py-2;
}
.zfile-video-tools {
  @apply grid grid-cols-3
        gap-0 bg-gray-50 p-3
        sm:flex sm:flex-wrap sm:justify-center sm:space-x-10;

  .zfile-video-tools-item {
    @apply m-2 flex-shrink-0 cursor-pointer rounded-md bg-white px-2 py-1 text-center shadow hover:shadow-2xl;

    img {
      @apply inline h-8 w-8;
    }
  }
}

.zfile-video-tools-tips {
  @apply bg-gray-50 pb-2 text-center text-sm text-gray-500;
}
</style>
