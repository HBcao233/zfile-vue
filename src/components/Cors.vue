<template>
  <div class="zfile-cors-wrapper">
    <el-button type="primary" @click="openDrawer('add', null)">新增配置</el-button>
    <el-tooltip
      content="检测到当前跨域配置异常，可能会影响在线预览、上传等功能，请点击添加推荐配置"
      placement="top"
    >
      <el-button v-show="!isRecommendConfig" type="success" @click="addRecommendConfig"
        >添加推荐配置</el-button
      >
    </el-tooltip>
    ...
    <el-drawer
      ...
      class="zfile-cors-drawer-wrapper"
      append-to-body
      title="跨域规则配置"
    ...
    >
      <template #header>
        <div class="zfile-cors-drawer-title">
          ...
          <a
            target="_blank"
            class="link my-auto text-xs"
            type="success"
            href="https://docs.aws.amazon.com/zh_cn/AmazonS3/latest/userguide/ManageCorsUsing.html"
          >
            参考文档
          </a>
        </div>
      ...
      >
        <el-form-item label="来源" prop="allowedOriginsStr">
          <el-input
            v-model="form.allowedOriginsStr"
            type="textarea"
            placeholder="请输入允许来源，如 https://xxx.com 或 http://192.168.0.1"
            :autosize="{ minRows: 3, maxRows: 6 }"
          />
          <div class="el-form-item-tips">
            <ul class="ml-4 list-disc">
              <li>
                来源可以设置多个，每行一个，每行最多能有一个通配符 *，每行前后不要后空格，最后不要加
                /
              </li>
              <li>
                如你有多个地址/域名访问该 ZFile
                服务(如家里服务器搭建，有内网，也有公网)都要配置到这里，不然可能会影响上传、下载、预览功能，后续增加其他地址访问时，记得更新这里。更详细的配置请参考存储源官方提供的文档。
              </li>
              <li>
                当前环境推荐配置:
                <span class="select-all">{{ currentOrigin }}</span>
              </li>
            </ul>
          </div>
        </el-form-item>
        ...
        <el-form-item label="允许 Headers" prop="allowedHeadersStr">
          <el-input
            v-model="form.allowedHeadersStr"
            type="textarea"
          ...
          <div class="el-form-item-tips">
            ...
            <br />
            ZFile 当前版本需要用
            <span class="select-all">content-type</span>，其他的根据需求添加。
          </div>
        </el-form-item>
        <el-form-item label="暴露 Headers" prop="exposeHeadersStr">
          <el-input
            v-model="form.exposeHeadersStr"
            type="textarea"
        ...
        <el-form-item label="缓存时间（秒）" prop="maxAgeSeconds">
          ...
          ></el-input-number>
          <div class="el-form-item-tips">
            指定在 S3 针对特定资源的预检 OPTIONS
            请求作出响应后，浏览器缓存该响应的时间（以秒为单位）。通过缓存响应，在需要重复原始请求时，浏览器无需向
            S3 发送预检请求。
          </div>
        </el-form-item>
...
const rules = {
  allowedOriginsStr: [{ required: true, message: '请输入允许来源', trigger: 'blur' }],
  allowedMethods: [
    {
      type: 'array',
      required: true,
...
const isRecommendConfig = computed(() => {
  return corsConfig.value.some((config) => {
    return (
      config.allowedOrigins.includes(currentOrigin) &&
