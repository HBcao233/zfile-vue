      ...
      <div v-if="isNotMobile" class="zfile-header-btn">
        ...
        <template v-if="storageConfigStore.globalConfig.debugMode">
          <el-tooltip placement="bottom">
            ...
            <el-button size="default" type="danger" @click="openResetAdminPwdDialog">
              重置管理员密码
            </el-button>
          </el-tooltip>
        </template>
        ...
        <el-dropdown v-if="storageConfigStore.loginInfo.isLogin" trigger="click">
          <i-mdi-account-circle-outline class="h-6 w-6 text-gray-500 hover:text-blue-500" />
          <template #dropdown>
            <el-dropdown-menu class="font-medium">
              <el-dropdown-item>
                <span class="mx-auto text-center text-gray-500"
                  >{{ storageConfigStore.loginInfo.username
                  }}{{
                    storageConfigStore.loginInfo.nickname
                      ? '(' + storageConfigStore.loginInfo.nickname + ')'
                      : ''
                  }}</span
                >
              </el-dropdown-item>
              <el-dropdown-item v-if="storageConfigStore.loginInfo.isAdmin" @click="toAdmin">
        ...
        <!-- 登录账号 -->
        <el-tooltip v-else-if="storageConfigStore.globalConfig.showLogin" placement="bottom">
          <template #content> 登录账号 </template>
          <div @click="toLoginView">
            <i-mdi-login-variant class="h-6 w-5 text-gray-500 hover:text-blue-500" />
          </div>
        </el-tooltip>
        ...
        <el-dropdown
          v-if="
            ...
            (storageConfigStore.permission.upload || storageConfigStore.permission.newFolder)
          "
          trigger="click"
        ...
        >
          ...
          <template #dropdown>
            <el-dropdown-menu class="font-medium">
              ...
              <el-dropdown-item
                v-if="storageConfigStore.permission.uploadFolder"
        ...
        >
          <i-solar-gallery-wide-linear v-if="fileDataStore.imgMode" class="h-6 w-5 text-blue-400" />
          <i-solar-gallery-wide-linear v-else class="h-6 w-5" />
        </div>

        <!-- 设置按钮 -->
        <div @click="openSettingVisible">
          <Cog8ToothIcon class="h-6 w-6"></Cog8ToothIcon>
        </div>
      </div>

      <!-- 存储源选择 -->
      <div v-if="isNotMobile" class="zfile-header-storage-select">
        <el-select v-model="currentStorageKey" size="default" placeholder="请选择存储源">
          <el-option
    ...
    <div v-if="isMobile">
      <el-dropdown trigger="click">
        ...
        <template #dropdown>
          <el-dropdown-menu>
            <el-dropdown-item
              v-if="storageConfigStore.globalConfig.debugMode"
              @click="openResetAdminPwdDialog"
            >
              <i-mdi-key class="mr-2 h-4 w-4 text-gray-500" name="key" />
              重置管理员密码
            </el-dropdown-item>
            <el-dropdown-item
              v-if="route.params.storageKey && storageConfigStore.permission.newFolder"
              @click="newFolder"
            >
              <i-lucide-folder-plus class="mr-2 h-4 w-4 text-gray-500" name="add-folder" />
              新建文件夹
            </el-dropdown-item>
            <el-dropdown-item
              v-if="route.params.storageKey && storageConfigStore.permission.upload"
              @click="openUploadDialog"
            >
              <i-ci-file-upload class="mr-2 h-4 w-4 text-gray-500" name="upload" />
              上传文件
            </el-dropdown-item>
            <el-dropdown-item
              v-if="route.params.storageKey && storageConfigStore.permission.uploadFolder"
              @click="openUploadFolderDialog"
            >
              <i-ci-folder-upload class="mr-2 h-4 w-4 text-gray-500" name="upload-folder" />
              上传文件夹
            </el-dropdown-item>
            <el-dropdown-item
              v-if="route.params.storageKey && !fileDataStore.imgMode"
              @click="fileDataStore.imgMode = true"
            >
              <i-ri-image-line class="mr-2 h-4 w-4 text-gray-500" name="image" />
              打开画廊模式
            </el-dropdown-item>
            <el-dropdown-item
              v-else-if="route.params.storageKey && fileDataStore.imgMode"
              @click="fileDataStore.imgMode = false"
            >
              <i-ri-image-line class="mr-2 h-4 w-4 text-blue-500" name="image" />
              关闭画廊模式
            </el-dropdown-item>
            <el-dropdown-item @click="openSettingVisible">
              <Cog8ToothIcon class="mr-2 h-4 w-4 text-gray-500"></Cog8ToothIcon>
              更多设置
            </el-dropdown-item>
            ...
            <template v-if="storageConfigStore.loginInfo.isLogin">
              <el-dropdown-item v-if="storageConfigStore.loginInfo.isAdmin" @click="toAdmin">
                <i-mdi-account-settings-variant class="mr-2 h-4 w-4 text-gray-500" name="admin" />
                前往后台
              </el-dropdown-item>
              <el-dropdown-item @click="openUpdatePwdDialog">
                <i-mdi-lock class="mr-2 h-4 w-4 text-gray-500" name="lock" />
                修改密码
              </el-dropdown-item>
              <el-dropdown-item @click="logout">
                <i-mdi-logout class="mr-2 h-4 w-4 text-gray-500" name="logout" />
                注销
              </el-dropdown-item>
            </template>
            <el-dropdown-item
              v-else-if="storageConfigStore.globalConfig.showLogin"
              @click="toLoginView"
            >
              <i-mdi-login-variant class="mr-2 h-4 w-4 text-gray-500" name="login" />
              登录账号
            </el-dropdown-item>
          </el-dropdown-menu>
        </template>
...
watch(
  () => [storageConfigStore.folderConfig.defaultSwitchToImgMode, fileDataStore.oldStorageKey],
  (val, oldValue) => {
...
.zfile-header {
  ...
  .zfile-header-right {
    ...
    .zfile-header-btn {
      @apply flex items-center space-x-10 text-4xl;

      div {
        @apply h-5 cursor-pointer #{!important};
      }
    }
