<template>
  <el-table
    ...
    :border="props.border"
    @sort-change="attr.onSortChange"
    @selection-change="selectionChange"
  >
    <slot></slot>
  </el-table>
  <div v-else class="el-table h-full w-full overflow-auto">
        ...
        <el-form label-position="top">
          ...
          <template v-for="citem in $slots['default']()" :key="citem">
            <template v-if="citem.props">
              ...
              <template v-else-if="citem.props.type === 'selection'"> </template>
...
import { ElTable, ElCard, ElForm, ElFormItem, ElDivider, ElCheckbox } from 'element-plus';
...
const isNeedSelection = computed(() => {
  ...
  return slots['default']().some(
    (item: any) => item && item.props && item.props.type === 'selection'
  );
});
