  ...
  <div class="zfile-index-body" @contextmenu="showFileMenu">
    <div
      ...
      :class="
        'zfile-index-table-' +
        (isMobile
          ? storageConfigStore.globalConfig?.mobileLayout
          : storageConfigStore.globalConfig?.layout)
      "
    >
      <!-- 公告 -->
      <el-alert
      ...
      <table-file-view v-if="zfileSettingCache.view.type === 'table'" class="z-file-view" />

      <card-file-view v-if="zfileSettingCache.view.type === 'card'" class="z-file-view" />
...
const loadUserRootPath = () => {
  return new Promise((resolve, reject) => {
    ...
    loadUserRootPathReq(route.params.storageKey)
      .then((res) => {
...
.zfile-index-table-card {
  ...
  &::after {
    content: '';
    @apply relative top-2 block h-2.5;
  }

  .z-file-view {
    @apply my-5 rounded-lg p-5 shadow-lg;
  }
}
</style>
