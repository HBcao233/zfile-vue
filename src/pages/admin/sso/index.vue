  ...
  <div class="zfile-admin-sso-setting-wrapper">
    <el-dropdown split-button type="primary" class="mb-2" @click="addTab('')">
      ...
      <template #dropdown>
        <el-dropdown-menu>
          <el-dropdown-item
            v-for="item in baseSsoConfig"
            :key="item.provider"
            @click="addTab(item.provider)"
          >
            {{ item.name }}
          </el-dropdown-item>
        </el-dropdown-menu>
    ...
    <div class="text-sm text-gray-400">
      注意：单点登录后新用户的默认权限取自
      <router-link to="/admin/user-list" class="link">用户管理</router-link> 中
      <router-link to="/admin/user-edit/0" class="link">虚拟新用户</router-link>
      的默认权限设置，请确保该用户有访问权限。
    </div>

    <el-empty v-if="ssoConfigList.length === 0" class="mt-20 h-full" description="暂无数据" />

    <el-tabs v-model="currentSsoProvider" closable @tab-remove="removeTab">
      <el-tab-pane v-for="item in ssoConfigList" :key="item.id" :label="item.name" :name="item.id">
        <el-form
          ref="ssoFormRef"
          :rules="formRules"
          :model="item"
        ...
        >
          ...
          <el-form-item label="服务商简称" prop="provider">
            ...
            <div class="el-form-item-tips">
              ...
              <span class="link select-all">{{
                concatPath(serverAddress, 'sso', item.provider, 'login/callback')
              }}</span>
            </div>
          </el-form-item>
          ...
          <el-form-item label="授权端点" prop="authUrl">
            ...
            <el-button class="mt-2" type="primary" @click="addByWellKnownEndpoint(item)"
              >通过 .well-known 地址自动获取</el-button
            >
          </el-form-item>
          ...
          <el-form-item label="Scope" prop="scope">
            ...
            <div class="el-form-item-tips">
              授权范围，请根据服务商的要求填写，应支持调用上面填写的用户信息接口
            </div>
          </el-form-item>
          <el-form-item label="用户字段" prop="bindingField">
            <el-input v-model="item.bindingField" placeholder="请输入" />
            <div class="el-form-item-tips">
              取用户信息端点返回的哪个字段作为用户名，支持
              <a
                class="link"
                href="https://alibaba.github.io/fastjson2/jsonpath_cn"
                target="_blank"
              >
                JSONPath 语法
              </a>
...
const addByWellKnownEndpoint = (item) => {
  ElMessageBox.prompt(
    '请在输入框中输入 .well-known 地址，系统会自动解析出相关参数。<br>如：https://gitlab.com/.well-known/openid-configuration',
    '提示',
    {
