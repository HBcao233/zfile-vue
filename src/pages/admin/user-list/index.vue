  ...
  <div class="z-admin-user-list">
    ...
    >
        ...
        <template #default="scope">
          <div class="flex justify-between space-x-2">
            <span>
              {{ scope.row.username }}
              <el-tooltip
                v-if="scope.row.id === newUserTemplateId"
                content="表示单点登录新用户或新注册用户默认的权限，如果关闭右侧的启用状态表示不允许新用户注册/单点登录新用户"
              >
                <QuestionMarkCircleIcon
                  v-if="scope.row.id === newUserTemplateId"
                  class="inline w-4 text-gray-500"
                />
              </el-tooltip>

              <el-tooltip v-if="scope.row.id === guestId" content="表示未登录用户可使用的权限">
                <QuestionMarkCircleIcon
                  v-if="scope.row.id === guestId"
                  class="inline w-4 text-gray-500"
                />
              </el-tooltip>
            </span>
            <span>
              <el-tag v-if="scope.row.id === newUserTemplateId" effect="dark" type="info"
                >虚拟新用户</el-tag
              >
              <el-tag v-else-if="scope.row.id === adminId" effect="dark">管理员</el-tag>
              <el-tag v-else-if="scope.row.id === guestId" effect="dark" type="success"
                >匿名用户</el-tag
              >
      ...
      <el-table-column-plus width="120" show-overflow-tooltip align="center" label="存储源列表">
        <template #default="scope">
          <el-popover
            :disabled="scope.row.userStorageSourceList.length === 0"
            placement="left"
            :width="300"
          >
            <template #reference>
            ...
            <div class="space-y-2">
              <div v-for="item in scope.row.userStorageSourceList" :key="item.id" class="space-x-2">
          ...
          <div class="flex space-x-2">
            ...
            <el-tooltip content="复制">
              <ClipboardDocumentIcon
                style="width: 1.2em"
                class="z-btn-primary"
                @click="openCopyUserDialog(scope.row)"
              />
            </el-tooltip>
            <el-popconfirm
              title="是否确认删除?"
...
.z-admin-user-list {
  :deep(.z-admin-search-from-row),
  :deep(.el-date-editor) {
    @apply w-full;
  }

  :deep(.z-admin-table-tools) {
    @apply pt-4;
  }
