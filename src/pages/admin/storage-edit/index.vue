  ...
  >
    ...
    <el-form-item label="存储源别名" prop="key">
      ...
      <div class="el-form-item-tips">
        存储源别名，用于 URL 中展示, 如
        {{ concatPath(serverAddress, storageItem.key || '{存储源别名}') }}
      </div>
    </el-form-item>
    ...
    <el-form-item label="是否启用">
      ...
      <div class="el-form-item-tips">如不启用，则在前台不显示，且不可访问.</div>
    </el-form-item>
    ...
    <div v-for="item in storageSourceParamList" :key="item.key">
      ...
      >
        ...
        >
          <el-option
            v-for="endPoint in constant.region[storageItem.type]"
            :key="endPoint.name"
            :label="endPoint.name"
            :value="endPoint.val"
          >
        ...
        >
          ...
          >
            <span style="float: left">
              {{ option.displayName }}
              <el-tag v-if="option.displayName === 'Communication site'" type="success"
                >默认网站</el-tag
              >
            </span>
            <el-popover placement="right" width="400" trigger="hover" :content="option.webUrl">
              <template #reference>
                <el-link
                  target="_blank"
                  :icon="LinkIcon"
        ...
        >
          ...
          >
            ...
            <el-popover placement="left" width="400" trigger="hover" :content="option.id">
              <template #reference>
        ...
        <!-- s3 跨域配置列表 -->
        <div v-else-if="item.key === 'corsConfigList'" class="w-full">
          <Cors v-model="storageItem.storageSourceAllParam['corsConfigList']" />
          <div class="text-sm text-gray-400" v-html="corsTips"></div>
        </div>
        ...
        <template v-else-if="item.type === 'select'">
          <el-select
            ...
            :allow-create="item.optionAllowCreate"
            :filterable="isNotMobile"
            default-first-option
          >
            <el-option
        ...
        <template v-else-if="item.type === 'input'">
          <el-input
            v-if="trimInputFields.includes(item.key)"
            v-model.trim="storageItem.storageSourceAllParam[item.key]"
            :disabled="inputIsReadOnly(item.key)"
          />
        ...
        <template v-else-if="item.type === 'textarea'">
          <el-input v-model="storageItem.storageSourceAllParam[item.key]" type="textarea" />
        </template>
        ...
        <div class="el-form-item-tips">
          ...
          <div v-if="item.link">
            <span v-if="item.key === 'accessToken'" class="flex items-center">
              <el-link
                ...
                :href="
                  item.link +
                  '?clientId=' +
                  encodeURIComponent(storageItem.storageSourceAllParam['clientId']) +
                  '&clientSecret=' +
                  encodeURIComponent(storageItem.storageSourceAllParam['clientSecret']) +
                  '&redirectUri=' +
                  encodeURIComponent(storageItem.storageSourceAllParam['redirectUri'])
                "
              >
                {{ item.linkName }}
              </el-link>
              <i-ic-baseline-content-copy
                class="relative ml-2 inline w-4 cursor-pointer text-lg"
                @click="
                  copyText(
                    item.link +
                      '?clientId=' +
            ...
            <el-link v-else target="_blank" :icon="LinkIcon" :href="item.link">{{
              item.linkName
            }}</el-link>
          </div>
          ...
          <div
            v-if="item.key === 'domain' && storageItem.storageSourceAllParam['domain']"
            class="font-bold"
          >
            注意:
            <ul>
              <li v-show="showSchemaTips" class="text-red-500">
                如果你的 zfile 是 https，则该存储源的下载域名也应是
                https，否则可能会影响打包下载和批量下载!
              </li>
          ...
          >
            ...
            <i-ic-baseline-content-copy
              ...
              @click="copyText(concatPath(serverAddress, 'onedrive/callback'))"
            />
          </div>
          <div
            v-if="
          ...
          >
            ...
            <i-ic-baseline-content-copy
              class="ml-1 inline cursor-pointer"
              @click="copyText(concatPath(serverAddress, 'onedrive/china-callback'))"
            />
          </div>
          <div
            v-if="
              item.key === 'redirectUri' && item.description && storageItem.type === 'google-drive'
            "
          >
            如：{{ concatPath(serverAddress, 'gd/callback') }}
            <i-ic-baseline-content-copy
              class="ml-1 inline cursor-pointer"
              @click="copyText(concatPath(serverAddress, 'gd/callback'))"
            />
          </div>
          <div v-if="item.key === 'accessToken' && storageItem.type === 'open115'">
            <open115-login
              :app-id="storageItem.storageSourceAllParam.clientId"
              @success="onOpen115LoginSuccess"
            />
          </div>
        </div>
      </el-form-item>
...
let useInitData = () => {
  ...
  let rules = ref({
    ...
    'storageSourceAllParam.url': [
      {
        type: 'url',
        message: '请输入正确的地址，需以 http:// 或 https:// 开头',
      },
    ],
...
let useS3Util = () => {
  ...
  const loadCorsConfig = (s3Credentials) => {
    loadS3CorsConfigReq(s3Credentials)
      .then((res) => {
        storageItem.value.storageSourceAllParam['corsConfigList'] = res.data;
        corsTips.value =
          '以上是自动获取到的跨域配置，如未正确配置跨域，可能会导致在线预览和上传失败。';
      })
      .catch((e) => {
        corsTips.value =
          '自动获取跨域配置失败，可能是存储源不支持/无需跨域配置，或者没有权限获取跨域配置或其他原因。<br>报错信息：' +
          e.response.data.msg;
      });
  };
          ...
          fieldShowMap.value[storageSourceParam.key] = computed(() => {
            const predicate = (conditionField, conditionValue, conditionSymbol) => {
              ...
              if (conditionSymbol === '==') {
                if (conditionValue === '') {
                  return (
                    storageItem.value.storageSourceAllParam[conditionField] === conditionValue ||
                    storageItem.value.storageSourceAllParam[conditionField] === null
                  );
                }
    ...
    if (newVal) {
      rules.value = {
        ...
        ...newVal.reduce((acc, item) => {
          if (item.required) {
            acc[`storageSourceAllParam.${item.key}`] = [
...
const showSchemaTips = computed(() => {
  return (
    ...
    storageItem.value.storageSourceAllParam['domain'].indexOf('http://') === 0
  );
});
...
.zfile-storage-edit {
  :deep(.el-form-item-input > .el-select) {
    width: 100%;
  }
}
</style>
