  ...
  >
    ...
    <el-form-item label="是否显示登录入口">
      ...
      <div class="el-form-item-tips">
        启用后，会在首页上方工具栏显示后台登录入口，请根据自身情况选择是否启用。(这里只是控制是否显示，直接访问登录页
        URL:
        <span class="text-blue-400">{{ currentLocale }}/login</span> 也能访问)
      </div>
    </el-form-item>

    <el-form-item label="匿名用户首页显示">
      <el-input v-model="siteSetting.guestIndexHtml" type="textarea" :rows="8"></el-input>
      <div class="el-form-item-tips">
        可填写 html 代码，用于强制匿名用户访问首页时显示的内容，留空则显示默认首页内容。<br />
        如强制跳转到登录页，可填写
        <code class="code select-all">&lt;script&gt;location.href = '/login';&lt;/script&gt;</code>
        <br />
    ...
    <el-form-item label="是否启用管理员双因素认证">
      <el-switch
        v-model="siteSetting.adminTwoFactorVerify"
        @change="adminTwoFactorVerifyOnChange"
      ></el-switch>
      <div class="el-form-item-tips">
        双因素认证可完美保护登录认证，可使用任意支持 2FA 认证的软件，如
        <b>Google 身份验证器</b>、<b>Microsoft Authenticator</b>
        等软件.（仅适用于管理员登录，不影响其他用户登录。）
      </div>
      <div v-if="siteSetting.adminTwoFactorVerify && siteSetting.loginVerifySecret" class="w-full">
    ...
    <el-form-item class="admin-from-footer">
      <el-button class="ml-auto" type="primary" :icon="CheckBadgeIcon" @click="beforeSave"
        >保存设置</el-button
      >
    </el-form-item>
  </el-form>
