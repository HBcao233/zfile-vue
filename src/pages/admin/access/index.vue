<template>
  ...
  >
    ...
    <el-form-item label="IP 黑名单">
      <el-input
        v-model="siteSetting.accessIpBlocklist"
        type="textarea"
        :rows="6"
        @input="testIpMatcher"
      />
      <div class="el-form-item-tips">
        ...
        <div class="mt-1">
          注意：检测到你当前 ip 为：<span class="text-blue-400">{{ clientIp }}</span
          >，别把自己 ip 配置进去（
          <el-popover placement="top-start" title="提示" :width="300" trigger="hover">
            <template #reference>
              <span
                >这里显示的不是你的 IP 地址？<QuestionMarkCircleIcon
                  class="inline h-4 w-4 align-sub"
              /></span>
            </template>
            <template #default>
              注意：如果你使用了反向代理，而不是直接访问的 ZFile 端口,
              那你需要在反向代理处添加以下任意请求头为用户真实 IP："X-Forwarded-For", "X-Real-IP",
        ...
        <div class="mt-4">
          ...
          <el-input v-model="testIpVal" @input="testIpMatcher">
            <template #suffix>
              <XCircleIcon v-if="testIpVal && testIpResult !== null" class="error-icon" />
              <CheckCircleIcon
                v-else-if="testIpVal && testIpResult === null"
                class="success-icon"
              />
            </template>
          </el-input>
          <div v-if="testIpVal">
            <div v-if="testIpResult">
              输入的测试 IP 匹配了规则:
              <span class="text-blue-500">{{ testIpResult }}</span
              >，会被禁止访问 ZFile。
            </div>
    ...
    <el-form-item label="UA 黑名单">
      ...
      <div class="el-form-item-tips">
        ...
        <div class="mt-1">
          注意：当前浏览器 UserAgent 为：<span class="text-blue-400">{{ userAgent }}</span
          >，别把自己 ua 填到黑名单去。
        </div>
        <div class="mt-4">
          <div>
            可在下方输入你要测试的 UserAgent, 显示绿色表示不会被拦截，红色表示触发了黑名单会被拦截
          </div>
          <el-input v-model="testUserAgentVal" @input="testUserAgentMatcher">
            <template #suffix>
              <XCircleIcon
                v-if="testUserAgentVal && testUserAgentResult != null"
                class="error-icon"
              />
              <CheckCircleIcon
                v-else-if="testUserAgentVal && testUserAgentResult === null"
                class="success-icon"
              />
            </template>
          </el-input>
          <div v-if="testUserAgentVal">
            <div v-if="testUserAgentResult">
              输入的测试 UserAgent 匹配了规则:
              <span class="text-blue-500">{{ testUserAgentResult }}</span
              >，会被禁止访问 ZFile。
            </div>
    ...
    <el-form-item label="快捷操作">
      <el-popover placement="top-start" title="提示" :width="300" trigger="hover">
        ...
        <template #default>
          <div>
            使用迅雷下载，可能会导致服务器CPU和带宽长时间被耗尽，所以这里提供了一键屏蔽迅雷下载的功能，点击按钮即可复制到剪贴板，然后粘贴到
