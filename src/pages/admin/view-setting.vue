<template>
  <div v-loading="siteSettingLoading" class="zfile-admin-view-setting-wrapper">
      ...
      <el-tab-pane label="使用习惯" name="a">
        ...
        >
          ...
          <el-form-item label="每次加载更多文件数">
            ...
            <div class="el-form-item-tips">当想显示更多文件时，每次额外显示的文件数</div>
          </el-form-item>
          ...
          <el-form-item label="默认排序字段">
            ...
            <div class="el-form-item-tips">默认排序方式, 用于控制文件夹中文件的显示顺序</div>
          </el-form-item>
          ...
          <el-form-item label="默认排序方式">
            ...
            <div class="el-form-item-tips">
              默认排序方式,
              用于控制文件夹中文件的显示顺序（文件和文件夹会分别排序，且始终是文件夹在前）
            </div>
          </el-form-item>

          <el-form-item label="显示目录文档">
            <el-switch v-model="siteSetting.showDocument" />
            <div class="el-form-item-tips">全局控制是否显示为存储源配置的目录文档</div>
          </el-form-item>
        </el-form>
      </el-tab-pane>
      ...
      <el-tab-pane label="预览类型" name="b">
        <div class="pt-4">
          <el-alert type="info">
            <template #title>
              <div>
                以下类型不区分大小，且不支持为空，为空会自动恢复至默认值，如果需要关闭某项的所有预览，可以随便写个不存在的格式。
              </div>
              <div>
                且这里填写了类型，不代表前端就能预览成功，这依赖浏览器是否支持该编码的，ZFile
                程序不负责解码或转码。简单的测试文件是否可在某浏览器预览的方式(不包含Office)是:
                新建一个浏览器标签页, 把文件拖进去, 看看是否能正常显示。
              </div>
            </template>
          </el-alert>
        </div>
              ...
              <template #suffix>
                <el-tooltip content="恢复默认">
                  <ArrowPathIcon
                    ...
                    @click="resetCustomSuffix('kkfileview')"
                  />
                </el-tooltip>
              </template>
            </el-input>
      ...
      <el-tab-pane label="OnlyOffice" name="c">
        ...
        >
          <el-form-item label="OnlyOffice 服务地址">
            ...
            <div class="el-form-item-tips">
              <div>
                OnlyOffice 服务地址，默认的公共服务不保证稳定性，推荐自行部署 OnlyOffice 服务.
                部署教程：<a
                  class="link"
              ...
              <ul class="ml-4 mt-2 list-disc">
                <li>
                  预览：进行预览的文件需 OnlyOffice 服务能访问到，如您的
                  OnlyOffice在公网，要预览的文件在内网，则无法正常预览。
                  <a
                    class="link"
                    target="_blank"
                    href="https://docs.zfile.vip/question/only-office-download-fail"
                  >
                    (为什么 OnlyOffice 预览失败？)
                  </a>
                </li>
                <li>
                  保存：因保存功能依赖于 OnlyOffice 回调 ZFile 服务，所以需您的 ZFile
                  服务能被OnlyOffice 访问到。
                </li>
                <li>
                  浏览器安全规范：如您的 ZFile 是 https 协议的，OnlyOffice 服务也必须是
                  https协议的，否则无法正常使用。
                </li>
              </ul>
            </div>
            <div v-if="loopbackAddressTipShow" class="mt-2 leading-none text-red-500">
              提示：检测到当前 ZFile 服务地址
              {{ currentHostname }} 属于环回地址，如果你的 OnlyOffice 部署在 Docker
              中，将无法正常进行预览，请切换为局域网地址或其他 OnlyOffice 在 Docker
              容器内可访问的地址。
            </div>
          </el-form-item>
          ...
          <el-form-item label="OnlyOffice Secret">
            ...
            <div class="el-form-item-tips">
              如果你部署的 OnlyOffice 需要开启 JWT Token 认证，可在此处填写
              Secret，否则可以留空。查看OnlyOffice Secret 文档：
              <a
                class="link"
                target="_blank"
                href="https://docs.zfile.vip/advanced/only-office#only-office-secret"
              >
        ...
        >
          <el-form-item label="kkFileView 服务地址">
            ...
            <div class="el-form-item-tips">
              <a
                class="link"
                href="https://www.kkview.cn/zh-cn/docs/production.html"
                target="_blank"
                >kkFileView 部署指南</a
              >
            </div>
            <div v-if="loopbackAddressTipShow" class="mt-2 leading-none text-red-500">
              提示：检测到当前 ZFile 服务地址
              {{ currentHostname }} 属于环回地址，如果你的 kkFileView 部署在 Docker
              中，将无法正常进行预览，请切换为局域网地址或其他 kkFileView 在 Docker
              容器内可访问的地址。
            </div>
          </el-form-item>
          ...
          <el-form-item label="自定义 JS">
            ...
            <div class="el-form-item-tips">
              ...
              <span class="text-red-400"
                >注意：如果你网站配置了防火墙，可能会被当成恶意请求拦截而保存失败。</span
              >
            </div>
          </el-form-item>
        </el-form>
...
const { siteSetting, siteSettingLoading, saveData, saveLoading } =
  useAdminSetting(updateViewSettingReq);
watch(
  () => siteSetting.value,
  (val, oldVal) => {
    if (oldVal === null) {
