<template>
      ...
      <el-tabs v-model="activeName">
        <el-tab-pane label="通用设置" name="a">
          <el-form-item label="记录下载日志">
            ...
            <div class="el-form-item-tips">是否记录直/短链下载日志</div>
          </el-form-item>
          ...
          <el-form-item
            v-show="siteSetting.refererType !== 'off'"
          ...
          >
            ...
            <div class="el-form-item-tips">
              ...
              <ul>
                ...
                <li>
                  限制泛域名
                  <span class="text-red-400">https 协议</span>
                  全部子目录可访问：
                  <div class="inline select-all font-bold">
              ...
              <div v-if="siteSetting.refererType === 'black_list'">
                每行输入一个域名，需要写协议头，支持 * 通配符，如黑名单 http://*zfile.vip
                将禁止所有如 http://zfile.vip、http://www.zfile.vip、http://demo.zfile.vip
                等网站访问。
              </div>
              <div class="mt-4">
                <div>
                  可在下方输入你要测试的地址，应包含协议和路径（如 http://example.com/1.html）
                </div>
                <el-input v-model="testAntVal" @input="testAntPathMatcher">
                  <template #suffix>
                    <XCircleIcon
                      v-if="testAntVal && testAntResult === null"
                      class="w-4 text-red-500"
                    ></XCircleIcon>
                    <CheckCircleIcon
                      v-else-if="testAntVal && testAntResult !== null"
                      class="w-4 text-green-500"
                    ></CheckCircleIcon>
                  </template>
                </el-input>
                <div v-show="testAntResult">
                  输入的测试地址匹配了规则:
                  <span class="text-blue-500">{{ testAntResult }}</span>
                </div>
              </div>
            </div>
          </el-form-item>
          ...
          <el-form-item label="下载频率限制">
            <div>
              <span>单 IP</span>
              <el-input-number
                v-model="siteSetting.linkLimitSecond"
                :min="0"
                :max="86400"
                :step="1"
                size="small"
              ...
              <span>秒内允许下载</span>
              <el-input-number
                v-model="siteSetting.linkDownloadLimit"
                :min="0"
                :max="9999999"
                :step="1"
                size="small"
              ...
              <el-button
                ...
                type="primary"
                class="ml-2"
                @click="openLinkLimitInfo"
                >统计信息
              </el-button>
            </div>
            <div class="el-form-item-tips">
              <span
                >用来防止恶意下载，单 IP N 秒内只允许访问 M 次直链，如其中一个为 0
                则不做任何限制.</span
              >
              <br /><br />
              <span
              ...
              <span
                ...
                那你需要在反向代理处设置以下请求头为用户真实 IP："X-Forwarded-For", "X-Real-IP",
                "Proxy-Client-IP", "WL-Proxy-Client-IP", "HTTP_CLIENT_IP",
                "HTTP_X_FORWARDED_FOR"，不然不论谁访问 ZFile 都只能获取到反代服务器的
                IP，同服务器一般是获取到 127.0.0.1 或 localhost, 这样此功能就无法正常使用!!!
                (辅助信息: 系统获取到您当前 IP 为
                <span class="select-all text-blue-400">{{ clientIp }}</span
                >)</span
              >
            </div>
          </el-form-item>
        ...
        <el-tab-pane label="直链设置" name="b">
          <el-form-item label="是否允许使用直链（路径）">
            ...
            <div class="el-form-item-tips">
              全局控制是否显示获取直链按钮及是否允许使用直链进行下载，<span class="font-bold"
                >优先级大于存储源权限设置.</span
              >
            </div>
          </el-form-item>
          ...
          <el-form-item label="直链地址前缀" prop="directLinkPrefix">
            ...
            <div class="el-form-item-tips">
              ...
              <span class="select-all"
                >{{ serverAddress
                }}<span class="font-bold text-red-400">{{ siteSetting.directLinkPrefix }}</span
                >/path/filename</span
              >
            </div>
          </el-form-item>
        ...
        <el-tab-pane label="短链设置" name="c">
          ...
          <el-form-item label="短链有效期">
            ...
            <div class="el-form-item-tips">控制生成短链的有效期</div>
          </el-form-item>
        </el-tab-pane>
      </el-tabs>
  ...
  <el-dialog v-model="showLinkLimitInfo" append-to-body title="统计信息" width="80%" top="5vh">
    <span
      >当前系统限制单 IP
      <span class="text-blue-400">{{ siteSetting.linkLimitSecond }}</span>
      秒内下载
      <span class="text-blue-400">{{ siteSetting.linkDownloadLimit }}</span>
      次，已记录
      <span class="text-blue-400">{{ linkLimitInfoList.length }}</span>
      条记录。（超出
      {{ siteSetting.linkLimitSecond }} 秒的记录将被清除。）</span
    >
    <ArrowPathIcon
      class="inline-block w-4 cursor-pointer hover:text-blue-400"
      @click="loadLinkLimitInfo"
    ></ArrowPathIcon>
    <el-table :data="linkLimitInfoList" max-height="400px">
      <el-table-column prop="key" label="IP"></el-table-column>
      <el-table-column prop="count" label="次数">
        <template #default="{ row }">
          <div :class="[row.value > siteSetting.linkDownloadLimit ? 'text-red-500' : '']">
            {{ row.value }}
          </div>
        </template>
      </el-table-column>
      <el-table-column prop="ttl" label="统计区间">
        <template #default="{ row }"> {{ row.ttl / 1000 }} 秒 </template>
      </el-table-column>
      <el-table-column prop="expiredTime" label="过期时间"></el-table-column>
      <template #empty>
        <div class="text-center text-gray-400">
          {{ siteSetting.linkLimitSecond }} 秒内没有任何下载记录
        </div>
      </template>
    </el-table>
    <template #footer>
...
const { siteSetting, saveData, saveLoading } = useAdminSetting(
  updateLinkSettingReq,
  settingFormRef
);
