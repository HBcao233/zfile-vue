<template>
  <div class="zfile-admin-down-link">
    <admin-form-header
      hide-bottom-border
      sub-title="这里只记录用户通过直链/短链下载的记录，不包括文件预览，下载失败(超出访问次数，超出访问时间，无权限等)"
    >
      <template #title>
        <div class="flex justify-between">
          ...
          <div
            v-if="siteSetting"
            class="flex cursor-pointer items-center justify-center space-x-1.5"
          >
            <span class="text-sm font-bold text-gray-400">记录下载日志：</span>
            <el-switch
              v-model="siteSetting.recordDownloadLog"
              active-text="是"
              inactive-text="否"
              inline-prompt
              @change="saveData"
      ...
      <el-row class="z-admin-search-from-row" :gutter="20">
        <el-col :span="globalConfigStore.adminSearch.spanSize">
          <el-form-item label="存储源" prop="storageKey">
            <el-select v-model="searchParam.storageKey" clearable placeholder="请选择存储源">
              <el-option
        ...
        <el-col :span="globalConfigStore.adminSearch.spanSize">
          <el-form-item label="下载类型" prop="linkType">
            <el-select v-model="searchParam.linkType" clearable placeholder="请选择下载类型">
        ...
        <el-col :span="globalConfigStore.adminSearch.spanSize">
          <el-form-item label="短链 Key" prop="shortKey">
            <el-input v-model="searchParam.shortKey" clearable></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="globalConfigStore.adminSearch.spanSize">
          <el-form-item label="文件路径" prop="path">
            <el-input v-model="searchParam.path" clearable></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="globalConfigStore.adminSearch.spanSize">
          <el-form-item label="请求 IP" prop="ip">
            <el-input v-model="searchParam.ip" clearable></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="globalConfigStore.adminSearch.spanSize">
          <el-form-item label="UserAgent" prop="userAgent">
            <el-input v-model="searchParam.userAgent" clearable></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="globalConfigStore.adminSearch.spanSize">
          <el-form-item label="Referer" prop="referer">
            <el-input v-model="searchParam.referer" clearable></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="globalConfigStore.adminSearch.spanSize">
        ...
        <template #default="scope">
          <div v-if="scope.row.shortKey">
            ...
            <i-ic-baseline-content-copy
              ...
              @click="copyText(scope.row.shortLink)"
            />
            <i-material-symbols-open-in-new
              class="ml-1 inline cursor-pointer text-sm text-blue-500"
              @click="openLink(scope.row.shortLink)"
            />
          </div>
          <div v-else>
            ...
            <i-ic-baseline-content-copy
              class="ml-2 inline cursor-pointer"
              @click="copyText(scope.row.pathLink)"
            />
            <i-material-symbols-open-in-new
              class="ml-1 inline cursor-pointer text-sm text-blue-500"
              @click="openLink(scope.row.pathLink)"
            />
          </div>
        </template>
      </el-table-column>
...
import {
  ...
  batchDeleteDownloadLogReqByQueryReq,
  deleteDownloadLogReq,
  getDownloadLogListReq,
} from '~/api/admin/admin-download-log';
...
const loadData = () => {
  loading.value = true;
  getDownloadLogListReq(searchParam.value)
    .then((res) => {
...
const batchDeleteLinkLogByQuery = () => {
  ElMessageBox.confirm(
    `是否确认删除当前查询条件下的 ${searchParam.value.total} 条直链下载日志？(如数量不对，请先点击查询后再用该功能按条件删除)`,
    '提示',
    {
      type: 'warning',
    }
