      ...
      <template #title>
        <div class="flex justify-between">
          ...
          <span @click="openSettingVisible">
            <AdjustmentsHorizontalIcon class="h-5 w-5 cursor-pointer text-blue-500" />
          </span>
        </div>
      </template>
    </admin-form-header>
    ...
    >
        ...
        <template #default="scope">
          <div class="space-x-2">
            ...
            <i-material-symbols-open-in-new
              class="inline cursor-pointer text-sm text-blue-500"
      ...
      <el-table-column width="180" prop="expireDate" label="过期时间">
        <template #default="scope">
          <div
            :class="
              dayjs(scope.row.expireDate).isBefore(dayjs().format('YYYY-MM-DD HH:mm'))
                ? 'text-red-500'
                : ''
            "
          >
...
import {
  batchDeleteShortLinkReq,
  deleteExpireLinkReq,
  deleteShortLinkReq,
  exportShortLinkListReq,
  getShortLinkListReq,
} from '~/api/admin/admin-short-link';
...
const loadData = () => {
  loading.value = true;
  getShortLinkListReq(searchParam.value)
    .then((res) => {
...
const batchExportLink = () => {
  ...
  exportShortLinkListReq(searchParam.value)
    .then((res) => {
