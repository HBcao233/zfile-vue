<template>
  ...
  >
    ...
    <el-form-item :size="globalConfigStore.adminForm.size" label="用户名" prop="username">
      <el-input v-model="data.username" :readonly="data.id === 2">
        <template #suffix>
    ...
    <el-form-item label="用户昵称" prop="nickname">
      <el-input v-model="data.nickname" :readonly="data.id === 2"> </el-input>
    </el-form-item>

    <el-form-item label="密码">
      <el-input v-model="data.password" autocomplete="off" type="password" show-password />
      <div class="el-form-item-tips">编辑时不显示密码，不修改密码请留空</div>
    </el-form-item>
    ...
    <el-form-item label="用户默认权限">
      <div>
        <el-button-group type="primary" class="mb-2">
          ...
          <el-button
            @click="
              data.defaultPermissions = permissionList
                .filter((item) => !data.defaultPermissions.includes(item.value))
                .map((item) => item.value)
            "
          >
        ...
        <el-checkbox-group
          v-model="data.defaultPermissions"
          class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5"
        >
          <el-checkbox v-for="item in permissionList" :key="item.value" :value="item.value"
            >{{ item.name }}
          </el-checkbox>
        </el-checkbox-group>
      </div>
    ...
    <el-form-item label="存储源授权">
      <el-button-group>
        ...
        <el-button
          ...
          size="small"
          class="mb-2"
          :disabled="getSelectedStorageSourceSize === 0"
          @click="openSetPermissionDialog(null)"
          >批量设置存储源列表
        </el-button>
      </el-button-group>
      <el-table
        ref="userStorageSourceTableRef"
        :size="globalConfigStore.adminTable.size"
        :data="data.userStorageSourceList"
        border
      >
        ...
        <el-table-column show-overflow-tooltip prop="rootPath">
          <template #header>
            ...
            <el-popover placement="right" width="300" trigger="hover">
              <template #default>
                表示该用户使用存储源下哪个目录作为根目录，注意要先创建好目录，否则使用时可能会出现问题，且不能填写
                URL 编码后的路径。
              </template>
              <template #reference>
          ...
          <template #default="scope">
            <el-input
              v-model="scope.row.rootPath"
              size="small"
              placeholder="请输入授权根目录"
            ></el-input>
          </template>
        </el-table-column>
        ...
        <el-table-column width="120px" prop="readOnly" label="权限列表">
          <template #default="scope">
            <div class="space-x-2">
              <el-popover
                :disabled="getSize(scope.row.permissions) === 0"
                placement="left"
                :width="10"
              >
                <template #reference>
                ...
                <div class="space-y-2">
                  <div v-for="item in scope.row.permissions" :key="item.id" class="space-x-2">
                    <el-tag>{{ getPermissionName(item) }}</el-tag>
                  </div>
                </div>
              </el-popover>
              <Cog6ToothIcon
                class="inline w-5 cursor-pointer text-blue-400"
  ...
  <el-dialog v-model="showSetPermissionDialog" append-to-body title="设置权限列表">
    <template #header>
      <span role="heading" aria-level="2" class="el-dialog__title">设置权限列表</span
      ...
      <div class="mb-5">
        <el-button
          type="primary"
          size="small"
          @click="currentOpenPermissions = permissionList.map((item) => item.value)"
        >
          全选
        </el-button>
        <el-button type="primary" size="small" @click="currentOpenPermissions = []"
          >全不选</el-button
        >
        <el-button
          type="primary"
          size="small"
          @click="
            currentOpenPermissions = permissionList
              .filter((item) => !currentOpenPermissions.includes(item.value))
              .map((item) => item.value)
          "
        >
          反选
        </el-button>
        <el-button type="primary" size="small" @click="setFromUserDefaultPermission"
          >设置为用户默认权限</el-button
        >
      </div>
    </template>
    ...
    <div class="zfile-user-permission-checkbox-body">
      <el-checkbox-group v-model="currentOpenPermissions" :size="globalConfigStore.adminForm.size">
        <el-checkbox
        ...
        >
          ...
          <el-popover placement="right" width="fit-content" trigger="hover">
            ...
            <template #reference>
              <QuestionMarkCircleIcon v-show="item.tips" class="-mt-0.5 ml-1 inline w-4" />
            </template>
          </el-popover>
        </el-checkbox>
      </el-checkbox-group>
    ...
    <template #footer>
      ...
      <el-button
        ...
        size="default"
        :icon="CheckBadgeIcon"
        @click="enterSetPermissionDialog"
        >确认</el-button
      >
    </template>
  </el-dialog>
...
const saveAction = () => {
  ...
  let errorIndex = data.value.userStorageSourceList.findIndex((item) => {
    return (
      ...
      item.rootPath === null ||
      item.rootPath === '' ||
      !item.rootPath.startsWith('/')
    );
  });
  if (errorIndex !== -1) {
    ElMessage.error(
      `第 ${errorIndex + 1} 个存储源的根目录配置错误，请检查。（不能为空，且必须以 / 开头）`
    );
    return;
  }
  ...
  adminUserFormRef.value.validate((valid) => {
    if (valid) {
      saveOrUpdateUserReq(data.value)
        .then((res) => {
...
let rules = ref({
  ...
  password: [{ min: 6, max: 255, message: '长度在 6 到 255 个字符', trigger: 'blur' }],
});
...
function useInitLoadData() {
  ...
  const completeStorageSourceList = () => {
    loadStorageListReq().then((res) => {
      ...
      let deleteStorageSourceIdList = userStorageSourceIdList.filter(
        (item) => !storageSourceIdList.includes(item)
      );
      let addStorageSourceIdList = storageSourceIdList.filter(
        (item) => !userStorageSourceIdList.includes(item)
      );
      // 删除掉
      deleteStorageSourceIdList.forEach((storageSourceId) => {
        let index = userStorageSourceList.findIndex(
          (item) => item.storageSourceId === storageSourceId
        );
...
const getSelectedStorageSourceSize = computed(() => {
  return userStorageSourceTableRef.value
    ? userStorageSourceTableRef.value.getSelectionRows().length
    : 0;
});
