...
const { openAudio, openImage, openOffice, openPdf, openText, openVideo, open3d, openKkFileView } =
  useFilePreview();
...
export default function useFileData() {
  ...
  const loadFile = (initParam) => {
    ...
    param.orderDirection =
      searchParam.orderDirection || storageConfigStore.globalConfig.defaultSortOrder;
  ...
  const openRow = (row, fromContextmenu) => {
    ...
    if (
      !fromContextmenu &&
      (contextMenuTargetFile.value === true || contextMenuTargetBlank.value === true)
    ) {
      return;
    }
