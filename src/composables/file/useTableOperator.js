...
const { selectRows, selectRow, clearSelection, toggleRowSelection, toggleAllSelection } =
  useFileSelect();
  ...
  const tableHoverRow = (row, column, cell, event) => {
    ...
    if (event.buttons === 1 && startDragClickIndex.value !== -1) {
      ...
      let oldEndDragClickIndex =
        endDragClickIndex.value >= 0 ? endDragClickIndex.value : newEndDragClickIndex - 1;
