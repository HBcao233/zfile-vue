...
import CompressBox from '~/components/file/compressBox';
...
export default function useFileOperator() {
  ...
  const copyTo = () => {
    if (
      ...
      !storageConfigStore.folderConfig.metadata.supportCopyFolder
    ) {
  ...
  const moveTo = () => {
    if (
      selectFolders.value.length > 0 &&
      !storageConfigStore.folderConfig.metadata.supportMoveFolder
    ) {
  ...
  const moveOrCopy = (title, reqFun) => {
    ...
    selectFolder(title, storageKey.value, currentPath.value, false, ({ value: path, callback }) => {
  ...
  const batchDelete = () => {
    ...
    if (
      selectFolders.value.length > 0 &&
      !storageConfigStore.folderConfig.metadata.supportDeleteNotEmptyFolder
    ) {
  ...
  const compress = () =>
    new Promise((resolve, reject) => {
      ...
      const lastIndex = basePath.lastIndexOf('/');
      const dirname = basePath.slice(lastIndex + 1);
      CompressBox(`<b>压缩</b> ${nameList.length}个项目`, dirname, ({ compressName, compressType, splitSizeMB, callback }) => {
        let param = {
          storageKey: storageKey.value,
          path: basePath,
          nameList: nameList,
  ...
  const decompress = () =>
    new Promise((resolve, reject) => {
      ...
      const basePath = currentPath.value;
      const fileName = selectRow.value.name;
      selectFolder(
        `<b>解压</b> ${fileName}`,
        storageKey.value,
        currentPath.value,
        true,
        ({ value: path, callback, createDir, password, charset }) => {
          let param = {
            storageKey: storageKey.value,
            path: basePath,
            name: fileName,
            ...
            password: password,
            charset: charset,
            srcPathPassword: getPathPwd(basePath),
            outputPassword: getPathPwd(path),
          };
