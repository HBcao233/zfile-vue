<template>
  ...
  <div class="h-full">
    <TransitionRoot as="template" :show="sidebarOpen">
      <Dialog as="menu" class="admin-menu" @close="sidebarOpen = false">
        <TransitionChild
          as="template"
    ...
    <div class="hidden lg:fixed lg:inset-y-0 lg:z-auto lg:flex lg:w-72 lg:flex-col">
      ...
      >
        <div class="flex h-16 shrink-0 items-center">
          <div class="flex flex-shrink-0 cursor-pointer items-center" @click="openHomePage">
            <img class="h-8 w-auto" src="../assets/icons/zfile-horizontal.svg" alt="ZFile" />
          </div>
          <div class="flex flex-shrink-0 cursor-pointer items-center" @click="openChangeLog">
            <el-badge class="word ml-1 hidden text-sm font-bold tracking-wider lg:block">
              {{ constant.version }}
            </el-badge>
          </div>
        </div>
        <nav class="flex flex-1 flex-col">
          <ul role="list" class="flex flex-1 flex-col gap-y-7">
            ...
            <li class="mb-5 mt-auto flex justify-center space-x-5 px-6">
              <a
              ...
              >
                <img class="h-8 w-8" src="../assets/icons/github.svg" />
              </a>
              <a
                class="cursor-pointer"
                title="ZFile 文档"
    ...
    <header class="admin-header">
      ...
      <div class="mx-auto flex space-x-2">
        ...
        <div class="flex flex-shrink-0 cursor-pointer items-center" @click="openChangeLog">
          <el-badge class="word hidden text-sm font-bold tracking-wider lg:block">
            {{ constant.version }}
          </el-badge>
        </div>
      </div>
...
const links = ref([
  {
    id: 1,
    name: '直/短链设置',
    href: '/admin/download-link/setting',
    icon: Cog8ToothIcon,
  },
  {
    id: 2,
    name: '短链管理',
    href: '/admin/download-link',
    icon: TableCellsIcon,
  },
  {
    id: 3,
    name: '下载日志',
    href: '/admin/download-link/log',
...
.admin-header {
  @apply sticky top-0 flex items-center gap-x-6 bg-white px-4 py-4 shadow-sm sm:px-6 lg:hidden z-[10000];
}

.admin-main-content {
  @apply h-full overflow-y-auto pb-16 bg-gray-50;

  .admin-main-content-container {
    @apply min-h-full bg-white shadow p-4 sm:mx-10 sm:h-auto sm:rounded-lg sm:px-6 lg:px-8;
  }
}
</style>
